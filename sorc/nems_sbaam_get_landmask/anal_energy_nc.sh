...
filelen=`cat ${target}.anl | wc -c`
if test $filelen -lt 1305
then
rm -f ${target}.anl
exit
fi

scp -p ${target}.anl wd53rl@cpcrzdm:ftp/new_sbaam/.
scp -p ${target}.anl roger.lin@${HOST1}:new_sbaam/.
scp -p ${target}.anl roger.lin@${HOST2}:new_sbaam/.

find . -maxdepth 1 -name '*.anl' -type f -mtime +14
find . -maxdepth 1 -name '*.anl' -type f -mtime +14 -exec rm -f {} \;
